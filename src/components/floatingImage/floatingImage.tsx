...
export default function FloatingImage({ img, type, width, alt, disableZoom = false }: Props) {
    ...
    switch (appliedType) {
        case 'left': {
            transfiguredStyle = { width: width, float: 'left' }
            break
        }
        case 'right': {
            transfiguredStyle = { width: width, float: 'right' }
            break
        }
        case 'center': {
            transfiguredStyle = {
    ...
    function onZoomIn() {
        setZoomBox(
            <Portal node={document && document.getElementById('image-zoom-box-placeholder')}>
                <div
                    onClick={onZoomOut}
                    className='fixed overscroll-none bg-white-80-alpha hover:cursor-zoom-out'>
                    <div className={'flex place-content-center h-screen w-screen px-6 py-6 pt-14'}>
                        {getImage({
                            maxWidth: '100%',
                            maxHeight: '100%',
